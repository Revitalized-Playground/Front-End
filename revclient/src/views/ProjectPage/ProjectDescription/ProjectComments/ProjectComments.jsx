	...
	const deleteComment = async (e, com) => {
		...
		if (deleted) {
			setProjectData({
				...
				project: {
					...projectData.project,
					comments: projectData.project.comments.filter(
						each => each.id !== deleted.data.deleteProjectComment.id,
					),
				},
			});
		}
	...
	return (
		<div className="projectCommentsContainer">
			...
			{localStorage.getItem('token') && (
				<form className="comment-form" onSubmit={submitComment}>
					...
					<button
						...
						style={comment.comment.length === 0 ? { color: 'gray', cursor: 'default' } : null}
					>
						Submit
					</button>
				</form>
			...
			<div className="comment-button-container">
				<button
					disabled={comments.length < commentCount ? true : false}
					style={
						comments.length < commentCount
							? { color: 'gray', border: '1px solid gray', cursor: 'default' }
							: null
					}
					className="see-more-comments"
					onClick={() => setCommentCount(commentCount + 8)}
				>
					{commentCount < comments.length ? 'Load more comments' : 'No more comments'}
				</button>
			</div>
		</div>
	);
